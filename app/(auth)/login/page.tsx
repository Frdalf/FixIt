...
import { Laptop, ArrowRight, ArrowLeft, Loader2 } from 'lucide-react'
...
function LoginForm() {
  ...
  useEffect(() => {
    if (searchParams.get('expired') === 'true') {
      ...
      // Remove expired from URL without reloading
  ...
  return (
    <Card className="border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm rounded-2xl">
      <CardHeader>
        <CardTitle className="text-2xl font-bold font-heading text-slate-800 dark:text-slate-100">Masuk Akun</CardTitle>
        <CardDescription className="text-slate-500 dark:text-slate-400">Masuk untuk memesan layanan servis laptop Anda</CardDescription>
      </CardHeader>
      <CardContent>
      ...
      <CardFooter className="flex flex-col space-y-4 text-sm text-center border-t border-slate-50 dark:border-slate-800 pt-6">
        ...
        <div className="w-full flex items-center justify-between text-xs text-slate-500 dark:text-slate-500 pt-2">
          <Link href="/teknisi/login" className="hover:underline flex items-center gap-1 text-slate-600 dark:text-slate-400">
            Area Teknisi <ArrowRight className="h-3 w-3" />
          </Link>
          <Link href="/admin/login" className="hover:underline flex items-center gap-1 text-slate-600 dark:text-slate-400">
    ...
    <div className="flex-1 flex flex-col items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 min-h-screen relative">
      ...
      <div className="w-full max-w-md">
        <div className="flex items-center justify-center gap-2 mb-6">
          <div className="bg-blue-900 dark:bg-blue-600 text-white p-2 rounded-xl">
            <Laptop className="h-6 w-6" />
          </div>
          <span className="text-2xl font-bold font-heading tracking-wider text-blue-900 dark:text-blue-400">FixIT</span>
        </div>
        ...
        <Suspense fallback={
          <Card className="border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm rounded-2xl p-6 flex flex-col items-center justify-center min-h-[300px]">
            <Loader2 className="h-8 w-8 animate-spin text-blue-900 dark:text-blue-400" />
            <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-2">Memuat halaman login...</span>
          </Card>
        }>
          <LoginForm />
        </Suspense>
