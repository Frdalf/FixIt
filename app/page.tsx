...
export default function LandingPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-100 bg-white/80 backdrop-blur-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
            <a href="#services" className="hover:text-blue-900 transition-colors">Layanan</a>
            <a href="#features" className="hover:text-blue-900 transition-colors">Keunggulan</a>
            <a href="#testimonials" className="hover:text-blue-900 transition-colors">Testimoni</a>
          </nav>
            ...
            {loading ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <Link href={profile?.role === 'admin' ? '/admin' : profile?.role === 'teknisi' ? '/teknisi/tasks' : '/repairs'}>
                  <Button className="bg-blue-900 hover:bg-blue-800 text-white rounded-xl text-xs sm:text-sm font-medium px-4">
                    Dashboard
                  </Button>
                </Link>
              </div>
              ...
              <>
                <Link href="/login">
                  <Button variant="ghost" className="text-slate-600 hover:text-blue-900 font-medium rounded-xl text-xs sm:text-sm">
                    Masuk
                  </Button>
                </Link>
                <Link href="/register">
      ...
      {/* Hero Section */}
      <section className="relative overflow-hidden bg-gradient-to-b from-blue-50/50 via-white to-white py-16 md:py-24 border-b border-slate-50">
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6 text-center md:text-left">
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-blue-50 border border-blue-100 text-xs font-semibold text-blue-900">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold font-heading text-slate-900 leading-tight">
              Laptop Rusak?<br />
              <span className="text-blue-900">Teknisi Kami Datang</span> Ke Tempat Anda!
            </h1>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-4">
              ...
              <a href="#services" className="w-full sm:w-auto">
                <Button variant="outline" className="w-full sm:w-auto border-slate-200 text-slate-600 hover:text-slate-900 font-semibold rounded-2xl py-6 px-8 text-base">
                  Lihat Layanan
                </Button>
              </a>
            </div>
          ...
          <div className="relative flex justify-center items-center">
            ...
            <div className="w-72 h-72 md:w-96 md:h-96 rounded-full bg-blue-100/50 absolute -z-10 blur-3xl animate-pulse" />
            <div className="bg-white border border-slate-100 shadow-xl rounded-3xl p-6 max-w-sm w-full space-y-4">
              <div className="flex items-center gap-3">
                <div className="bg-amber-100 text-amber-600 p-2 rounded-xl">
                  <Star className="h-5 w-5 fill-amber-500 stroke-amber-500" />
                </div>
                <div>
                  <div className="text-xs text-slate-500 font-medium">Ulasan Pengguna</div>
                  <div className="text-base font-bold text-slate-800">4.9/5.0 Rating Kepuasan</div>
                </div>
              </div>
              <div className="border-t border-slate-50 pt-4 flex items-center justify-between text-xs text-slate-600">
      ...
      <section id="services" className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <h2 className="text-3xl md:text-4xl font-extrabold font-heading text-slate-900">Layanan Perbaikan Kami</h2>
            <p className="text-slate-600">Kami menyediakan berbagai layanan perbaikan laptop terlengkap yang dikerjakan oleh teknisi ahli langsung di hadapan Anda.</p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {CATEGORIES.map((cat) => (
              <Card key={cat.slug} className="border-slate-100 hover:border-blue-100 hover:shadow-md transition-all rounded-2xl group cursor-pointer flex flex-col justify-between">
                <CardContent className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                  <div className="space-y-4">
                    <div className={`p-3 rounded-xl w-fit border ${cat.color}`}>
                      <cat.icon className="h-6 w-6" />
                    </div>
                    <h3 className="text-xl font-bold text-slate-800 group-hover:text-blue-900 transition-colors font-heading">{cat.name}</h3>
                    <p className="text-sm text-slate-600 leading-relaxed">{cat.description}</p>
                  </div>
                  <Link href={user ? `/repairs/${cat.slug}` : '/login'} className="inline-flex items-center gap-1 text-sm font-semibold text-blue-950 hover:underline pt-4 group-hover:gap-2 transition-all">
      ...
      {/* Features Section */}
      <section id="features" className="py-16 md:py-24 bg-slate-50/50 border-y border-slate-50">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <h2 className="text-3xl md:text-4xl font-extrabold font-heading text-slate-900">Mengapa Memilih FixIT?</h2>
            <p className="text-slate-600">Solusi perbaikan laptop modern, efisien, dan terpercaya tanpa perlu macet di jalan menuju toko servis.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-4 text-center">
              <div className="bg-blue-50 text-blue-900 p-4 rounded-full w-fit mx-auto">
                <MapPin className="h-6 w-6" />
              </div>
              <h3 className="text-lg font-bold text-slate-800 font-heading">On-Demand Terdekat</h3>
              <p className="text-sm text-slate-600">Algoritma kami mendeteksi lokasi Anda dan menugaskan teknisi terdekat untuk mempercepat kedatangan.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-4 text-center">
              <div className="bg-amber-50 text-amber-500 p-4 rounded-full w-fit mx-auto">
                <ShieldCheck className="h-6 w-6" />
              </div>
              <h3 className="text-lg font-bold text-slate-800 font-heading">Bergaransi & Transparan</h3>
              <p className="text-sm text-slate-600">Semua servis disertai garansi resmi FixIT. Harga tercantum di aplikasi transparan tanpa ada biaya tambahan siluman.</p>
            </div>
            <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm space-y-4 text-center">
              <div className="bg-indigo-50 text-indigo-600 p-4 rounded-full w-fit mx-auto">
                <Clock className="h-6 w-6" />
              </div>
              <h3 className="text-lg font-bold text-slate-800 font-heading">Lacak Real-Time</h3>
              <p className="text-sm text-slate-600">Pantau pergerakan status teknisi Anda mulai dari keberangkatan, proses servis, hingga laptop selesai diperbaiki.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="testimonials" className="py-16 md:py-24 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <h2 className="text-3xl md:text-4xl font-extrabold font-heading text-slate-900">Ulasan Pelanggan</h2>
            <p className="text-slate-600">Apa kata mereka yang telah menggunakan layanan perbaikan laptop on-demand FixIT.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, idx) => (
              <Card key={idx} className="border-slate-100 shadow-sm rounded-2xl p-6 bg-slate-50 flex flex-col justify-between">
