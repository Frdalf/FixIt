...
export default function TeknisiLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex flex-col min-h-screen pb-20 md:pb-0 bg-slate-50 dark:bg-slate-950">
      {/* Desktop Header */}
      <header className="hidden md:block sticky top-0 z-40 w-full border-b border-slate-100 dark:border-slate-900 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <Link href="/teknisi/tasks" className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold font-heading text-slate-800 dark:text-slate-200">
              FixIT <span className="text-xs font-semibold text-amber-600 bg-amber-50 dark:bg-amber-950/50 dark:text-amber-400 px-2 py-0.5 rounded-md">Teknisi</span>
            </span>
          </Link>
          <div className="flex items-center gap-6">
            <nav className="flex items-center gap-8 text-sm font-medium text-slate-605 dark:text-slate-400">
              {NAV_ITEMS.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  'flex flex-col items-center gap-1 text-[10px] font-medium transition-colors text-slate-550 py-1 px-3 rounded-xl',
                  isActive ? 'text-amber-600 dark:text-amber-400 font-semibold bg-amber-50/50 dark:bg-amber-950/20' : 'hover:text-slate-850 dark:hover:text-slate-200'
                )}
              >
                <item.icon className={cn('h-5 w-5', isActive ? 'text-amber-500' : 'text-slate-400 dark:text-slate-500')} />
