  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-4 bg-slate-50 min-h-screen">
        ...
        <span className="text-sm text-slate-500 font-medium mt-2">Memuat profil teknisi...</span>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-md pb-24">
      <div className="space-y-6">
        <h1 className="text-2xl font-extrabold font-heading text-slate-800">Profil Teknisi</h1>
        ...
        <Card className="border-slate-100 rounded-2xl shadow-sm bg-white overflow-hidden">
          ...
          <CardContent className="p-6 pt-0 relative">
            {/* Avatar */}
            <div className="h-16 w-16 rounded-full bg-blue-900 text-white font-extrabold text-2xl flex items-center justify-center border-4 border-white absolute -top-8 left-6 shadow-sm">
            ...
            <div className="pt-10">
              ...
              <span className="text-[10px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-full capitalize mt-1 inline-block">
                Mitra Teknisi
              </span>
            </div>

            {/* Stats grid */}
            <div className="grid grid-cols-2 gap-4 border-y border-slate-100 py-4 my-5 text-center">
              <div className="space-y-1">
                ...
                <div className="text-[10px] text-slate-500 uppercase font-semibold">Rating Rata-Rata</div>
              </div>
              <div className="space-y-1 border-l border-slate-100">
                <div className="text-blue-900 flex items-center justify-center gap-1 font-bold text-lg">
                  <Award className="h-5 w-5" />
                  {techProfile?.total_jobs || '0'}
                </div>
                <div className="text-[10px] text-slate-500 uppercase font-semibold">Pekerjaan Selesai</div>
              </div>
            </div>
            ...
            <div className="space-y-3.5 text-sm text-slate-650">
              ...
              <div className="pt-1">
                <div className="text-xs text-slate-500 font-semibold mb-2">Spesialisasi Keahlian:</div>
                <div className="flex flex-wrap gap-1.5">
                  {techProfile?.specializations?.map((spec: string) => (
                    <span
                      key={spec}
                      className="text-[10px] font-bold text-blue-900 bg-blue-50 border border-blue-100 px-2.5 py-0.5 rounded-full capitalize"
                    >
                      {spec}
                    </span>
                  ))}
